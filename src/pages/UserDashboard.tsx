...
import { Car, Clock, MapPin, CreditCard, History, Settings, User as UserIcon } from 'lucide-react';
...
const UserDashboard: React.FC = () => {
  ...
  const upcomingRides = userRides.filter(ride => 
    ['pending', 'confirmed', 'driver_assigned', 'arrived', 'in_progress'].includes(ride.status)
  );
      ...
      <div className="p-4">
        <div className="flex justify-between items-start">
            ...
            <div className="space-y-2">
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-800 dark:text-white">{ride.pickupLocation.address}</div>
                </div>
              </div>
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-800 dark:text-white">{ride.dropLocation.address}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="text-right">
            <div className="text-lg font-bold text-gray-900 dark:text-white">${ride.fare.toFixed(2)}</div>
            <div className="text-sm text-gray-500 dark:text-gray-400">{ride.distance} km</div>
          </div>
        </div>
        
        <div className="mt-4 flex justify-between items-center">
          <div className="text-sm text-gray-500 dark:text-gray-400">
            <span className="capitalize">{ride.cabType}</span> • {ride.paymentMethod.toUpperCase()}
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Link to="/book">
                  <Card interactive className="p-6 text-center">
                    ...
                    <h3 className="font-medium text-gray-900 dark:text-white">Book a Ride</h3>
                  </Card>
                </Link>
                
                <Card interactive className="p-6 text-center" onClick={() => setActiveTab('upcoming')}>
                ...
                <Card interactive className="p-6 text-center" onClick={() => setActiveTab('history')}>
                  <History className="h-8 w-8 text-green-600 dark:text-green-400 mx-auto mb-3" />
                  <h3 className="font-medium text-gray-900 dark:text-white">Ride History</h3>
                </Card>
              </div>
            </div>
