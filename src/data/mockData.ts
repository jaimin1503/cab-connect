...
export const mockDrivers: Driver[] = [
  ...
  ...Array(5)
    ...
    .map((_, index) => ({
      ...
      rating: parseFloat(
        faker.number.float({ min: 3.5, max: 5, precision: 0.1 }).toFixed(1)
      ),
...
export const mockRides: Ride[] = [
  ...
  ...Array(10)
    ...
    .map((_, index) => {
      ...
      const cabType = faker.helpers.arrayElement([
        "economy",
        "standard",
        "luxury",
      ]) as CabType;
      ...
      const ride: Ride = {
        ...
        distance: parseFloat(
          faker.number.float({ min: 1, max: 20, precision: 0.1 }).toFixed(1)
        ),
      ...
      if (
        ["driver_assigned", "arrived", "in_progress", "completed"].includes(
          status
        )
      ) {
        ride.driverId =
          mockDrivers[
            faker.number.int({ min: 0, max: mockDrivers.length - 1 })
          ].id;
      }

      if (status === "completed") {
        ride.startTime = new Date(
          Date.now() - faker.number.int({ min: 3600000, max: 86400000 })
        );
        ride.endTime = new Date(
          ride.startTime.getTime() + ride.duration * 60000
        );
        ride.rating = faker.number.int({ min: 1, max: 5 });
        ride.feedback = faker.helpers.maybe(() => faker.lorem.sentence(), {
          probability: 0.7,
        });
      }
      ...
      if (["pending", "confirmed", "driver_assigned"].includes(status)) {
        ...
        futureTime.setHours(
          futureTime.getHours() + faker.number.int({ min: 1, max: 24 })
        );
      ...
      if (status === "in_progress") {
        ride.startTime = new Date(
          Date.now() - faker.number.int({ min: 600000, max: 1800000 })
        );
        ride.routeDeviation = faker.helpers.maybe(() => true, {
          probability: 0.2,
        });
      }
